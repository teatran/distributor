...
public class InsertProductDialog extends JDialog
{
	...
	JLabel label_SEARCH_PRODUCT = new JLabel();
	...
	public InsertProductDialog(final ProductsFrame owner, final ProductsTableModel productsModel)
	{
		...
		suppliersTable.getSelectionModel().addListSelectionListener(new ListSelectionListener()
		{
			...
			public void valueChanged(ListSelectionEvent event) {
				...
				if (indexSelect != -1) {
					selectedSupplier = suppliersTable.getValueAt(indexSelect, 0).toString();
		...
		button_OK.addActionListener(new ActionListener()
		{
			...
			public void actionPerformed(ActionEvent arg0)
			{
				...
				productsModel.addRow(new Object[] {
						newProductName, newSupplierName, newQuantiy});
				
				// Insert a row into database
				try (Connection connection = new JDBCUtils().getConnection()) {
					try (Statement stat = connection.createStatement()) {
						stat.executeUpdate("insert into PRODUCTS values ('" +
								newProductName + "', '" + newSupplierName +
								"', '" + newQuantiy + "')"); 			
					}
				...
				// Update the label_PRODUCT_COUNT
				owner.label_PRODUCT_COUNT.setText(
						"Tổng số: " + productsModel.getRowCount() + " sản phẩm");
				
				InsertProductDialog.this.setVisible(false);
			}
		});
